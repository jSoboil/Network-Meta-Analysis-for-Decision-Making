...
t <- array(list(
 c(rep(1, 19), rep(2, 3), rep(3, 14)),
 c(3, rep(2, 8), 4, 5, rep(7, 11), 5, 5, 6, rep(7, 11))))
# Numerators and denominators arm [, k] of trial [i, ]:
# Arm 1 for trial i:
r <- array(list(
 c(1472, 3, 12, 7, 10, 887, 5, 1455, 9, 4, 285, 11, 1, 8, 1, 4, 14, 9, 42,
   2, 13, 2, 13, 356, 522, 3, 10, 40, 5, 5, 2, 19, 59, 5, 16, 8),
 c(652, 3, 7, 4, 5, 929, 2, 1418, 6, 6, 270, 2, 3, 5, 1, 0, 7, 3, 29, 3, 5,
   2, 7, 757, 523, 1, 3, 32, 5, 3, 3, 20, 52, 2, 12, 6)))
n <- array(list(
 c(20251, 65, 159, 85, 135, 10396, 63, 13780, 130, 107, 3004, 149, 50, 58,
   53, 45, 99, 41, 421, 44, 200, 56, 155, 4921, 8488, 55, 94, 573, 75, 69,
   61, 419, 782, 81, 226, 66),
 c(10396, 64, 157, 86, 135, 10372, 59, 13746, 123, 109, 3006, 152, 50, 54,
   47, 42, 101, 46, 429, 46, 195, 47, 169, 10138, 8461, 55, 95, 565, 75,
   71, 62, 421, 790, 81, 225, 71)))
n_s <- 36
n_t <- length(unique(t[[2]]))
# matrix of data:
array(list(n_arms, n_s, n_t, t, r, n),
      dimnames = list(c("n_arms", "n_s", "n_t", "t", "r", "n")))
